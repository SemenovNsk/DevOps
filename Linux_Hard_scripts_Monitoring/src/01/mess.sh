#!/bin/bash

source validate.sh

function create_dirs {
    touch log.txt
    for (( number=0; number <$count_directories; number++ ))
    do
        dir_name=""
        create_directory_name
        create_files
    done
}

function create_directory_name {
    if [[ ${#dir_name} -le 235 ]] && [[ ${#dir_name} -ne "" ]]; then
        mkdir ${path}$dir_name
        echo $path"/"$dir_name --- $(date +'%e.%m.%Y') ---  >> log.txt
    else
    generate_name "$letters_directories"
    create_directory_name 
    fi
}

function generate_name {
    for (( i=0; i<${#1}; i++ ))
    do
        letter=${1:i:1}
        for (( j=0; j<$((RANDOM%240+4)); j++ ))
        do
            if [[ ${1} = "$letters_directories" ]]; then
            dir_name+=$letter
            else
            file_name+=$letter
            fi
        done
    done
        if [[ ${1} = "$file_name_start" ]]
        then
            file_name+='.'
            for (( i=0; i<${#2}; i++ ))
            do
                letter=${2:i:1}
                for (( j=0; j<$((RANDOM%240+4)); j++ ))
                do
                    file_name+=$letter
                done
            done
        fi
    if [[ ${1} = "$letters_directories" ]]; then
    dir_name+="_"
    dir_name+=$(date +"%d%m%y")
    else
    file_name+="_"
    file_name+=$(date +"%d%m%y")
    fi
}

function create_files {
   
    for (( number_files=0; number_files <$count_files; number_files++ ))
    do
        file_name=""
        create_files_name
    done
}

function create_files_name {

    file_name_start="$(echo $letters_files | awk -F "." '{print $1}')"
    file_name_end="$(echo $letters_files | awk -F "." '{print $2}')"

    if [[ ${#file_name} -le 235 ]] && [[ ${#file_name} -ne "" ]]; then
    if [[ $(df / -BM | grep "/" | awk -F"M" '{ print $3 }') -le 1024 ]]
    then
        exit 1
    fi
        fallocate -l $size"KB" ${path}$dir_name/$file_name
        echo $path"/"$dir_name"/"$file_name-$size "Kb" >> log.txt
        echo $path"/"$dir_name"/"$file_name --- $(date +'%e.%m.%Y') --- $size "Kb" >> log.txt
    else
    generate_name "$file_name_start" "$file_name_end"
    create_files_name 
    fi
}

